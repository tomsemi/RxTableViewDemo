...
class MultipleRxTableViewController: UIViewController {
    ...
    let dataSource = RxTableViewSectionedReloadDataSource<SectionModel<String, Int>>(configureCell: { (dataSource, table, indexPath, _) in
...
extension MultipleRxTableViewController {
    ...
    func bindWithVariables() {
        for _ in tableViews {
            dataSources += [RxTableViewSectionedReloadDataSource<SectionModel<String, Int>>(configureCell: { (dataSource, table, indexPath, _) in
                return UITableViewCell()
            })]
            variables += [Variable([])]
        }
